...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (course) {
      ...
      res
        .status(404)
        .json({ message: `No course found for id: ${req.params.id}` });
    }
  })
);
  ...
  asyncHandler(async (req, res) => {
    ...
    if (validationErrors.length === 0) {
      ...
      /* this route is only available if user is properly authenticated
       so res.locals.user.userId will always be defined */
  ...
  asyncHandler(async (req, res) => {
    ...
    if (course) {
      ...
      will only update those keys */
        ...
        // user has no permission to edit the course
        res.status(403).end();
      }
    }
    res.status(204).end();
